...
function get_default_ai_prompt_instruction(): string
{
    return implode("\n", [
        ...
        "以下のユーザ情報と会話ログを読み取り、相手の特徴・温度感・直近のやり取り内容を分析したうえで、自然な状況確認メッセージを1通だけ作成してください。",
        "分析観点:",
        "- 相手の特徴（返信頻度、文章量、感情の温度感、前向き/不安/停滞など）を把握する",
        "- 直近で取り組んでいる内容、困っていること、止まっている箇所、未返信の流れを整理する",
        "- 担当者との関係性や過去の声かけのトーンに合わせる",
        "作成条件:",
        "- 日本語で、LINEにそのまま貼り付けられる文面にする",
        "- 相手を責めず、圧をかけすぎず、前向きで返信しやすい文面にする",
        "- 直近の具体的な状況に1つ触れてから、今の状況を確認する",
        "- 必要に応じて、短く答えられる質問を1つ入れる",
        "- 180文字以内を目安にする",
        "- 分析内容、件名、説明、候補リスト、引用符は付けず、送信文のみを返す"
    ]);
}
